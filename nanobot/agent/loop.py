...
class AgentLoop:
    ...
    async def _process_message(
        ...
        # For ordinary turns the current user message may be merged into the
        # last visible user history item for provider role-alternation.  Skip
        # the actual prompt prefix length so a dangling user-only tail cannot
        # make us skip the assistant response too.
        if pending_ask_id:
            save_skip = 1 + len(history)
        elif user_persisted_early:
            save_skip = initial_message_count
        else:
            save_skip = 1 + len(history)
