...
class TestDreamRun:
    ...
    async def test_syncs_history_index_even_when_no_new_dream_entries(
        self, dream, mock_provider, mock_runner, store,
    ):
        """Dream's maintenance tick should refresh the FTS sidecar before no-op."""
        store.append_history("User prefers dark mode")
        store.append_history("Project uses PostgreSQL")
        ...
        assert store.search_history("postgresql")[0]["cursor"] == 2
        mock_provider.chat_with_retry.assert_not_called()
        mock_runner.run.assert_not_called()

    async def test_phase1_prompt_includes_related_older_history(
        self, dream, mock_provider, mock_runner, store,
    ):
        """Dream can use the index to add relevant older history to Phase 1."""
        store.append_history("Solved auth token refresh by rotating credentials")
        store.set_last_dream_cursor(1)
        store.append_history("Auth token refresh failed again in production")
        ...
        assert "## Related Older History" in user_msg
        assert "Solved auth token refresh" in user_msg
