...
class TestHistoryIndex:
    def test_append_history_does_not_update_index_realtime(self, store):
        store.append_history("User prefers dark mode")

        assert not store.history_index_file.exists()
        assert store.get_last_index_cursor() == 0

    def test_sync_history_index_batches_entries_and_searches(self, store):
        store.append_history("User prefers dark mode")
        store.append_history("Project decided to use PostgreSQL")

        indexed = store.sync_history_index()

        assert indexed == 2
        assert store.history_index_file.exists()
        assert store.get_last_index_cursor() == 2
        results = store.search_history("postgresql", limit=5)
        assert [r["cursor"] for r in results] == [2]
        assert "PostgreSQL" in results[0]["content"]

    def test_search_history_scans_unindexed_tail(self, store):
        store.append_history("Project decided to use PostgreSQL")
        store.sync_history_index()
        store.append_history("Later user fixed token refresh")

        results = store.search_history("token refresh", limit=5)

        assert [r["cursor"] for r in results] == [2]
        assert store.get_last_index_cursor() == 1

    def test_search_history_falls_back_when_index_missing(self, store):
        store.append_history("User prefers concise Chinese replies")

        results = store.search_history("Chinese", limit=5)
