...
- `memory/history_index.db` is a rebuildable SQLite FTS index for faster history search.
...
└── memory/
    ...
    ├── history_index.db # Rebuildable SQLite FTS search index
    ├── .cursor          # Consolidator write cursor
    ├── .index_cursor    # History index consumption cursor
...
- `history_index.db` accelerates search, but `history.jsonl` remains the source of truth.
...
`history_index.db` is deliberately not updated on every chat turn. It is
refreshed in batches during memory maintenance, before Dream processes new
history. If it is missing or damaged, nanobot can rebuild it from
`history.jsonl`.
